...
const DAY=86400000;
function isHard(){return mode==='hard'||mode==='hardPractice'}
function isPractice(){return mode==='practice'||mode==='hardPractice'}
function pool(){return isHard()?HARD:CLASSIC}
...
function dailyAnswer(){const n=Math.max(1,puzzleNumber()), a=isHard()?hardShuffled:classicShuffled;return a[(n-1)%a.length]}
function stateKey(){return isHard()?`rydle_hard_daily_${dateKey()}`:`stcdle_daily_${dateKey()}`}
...
function randomPractice(){const P=pool();let next;do{next=P[Math.floor(Math.random()*P.length)]}while(P.length>1&&next?.id===answer?.id);answer=next;guesses=[];finished=false;selected=null}
...
function setBoardMode(){const hard=isHard();board.classList.toggle('hard-board',hard);headers.innerHTML=hard?'<div>Castaway</div><div>Season</div><div>Place</div><div>Gender</div><div>Color</div><div>Returnee</div><div>Format</div>':'<div>Castaway</div><div>Season</div><div>Place</div><div>Gender</div><div>Color</div><div>Returnee</div>'}
function render(){
 ...
 const count=isHard()?7:6;for(let i=guesses.length;i<MAX;i++){const r=document.createElement('div');r.className='guess-row';for(let j=0;j<count;j++){const d=document.createElement('div');d.className='cell empty';r.appendChild(d)}rows.appendChild(r)}
 const won=guesses.at(-1)?.id===answer.id;
 $('#puzzleMeta').textContent=mode==='daily'?`Rydle #${Math.max(1,puzzleNumber())} · ${dateKey()}`:mode==='hard'?`Rydle Hard #${Math.max(1,puzzleNumber())} · ${dateKey()}`:mode==='hardPractice'?'Random Hard practice puzzle':'Random Classic practice puzzle';
 $('#modeDescription').textContent=mode==='daily'?'One full-season appearance. Eight guesses. Same puzzle for everyone today.':mode==='practice'?'Unlimited random Classic puzzles. Practice games do not affect your streak.':mode==='hard'?'Every STC event counts. Full seasons and minis. Eight guesses. Same Hard puzzle for everyone today.':'Unlimited random Hard puzzles from every STC event. Practice games do not affect your streak.';
 document.querySelectorAll('.tab').forEach(b=>b.classList.toggle('active',b.dataset.mode===mode));input.disabled=finished;$('#guessBtn').disabled=finished;$('#newPracticeBtn').classList.toggle('hidden',!isPractice());
 if(finished){endPanel.classList.remove('hidden');$('#endTitle').textContent=won?'You survived!':'The tribe has spoken.';$('#endAnswer').innerHTML=`The answer was <strong>${answer.displayName}</strong> — ${placeText(answer)}, ${answer.startingTribe} (${answer.tribeColor}).`;$('#nextPuzzle').textContent=isPractice()?'':'Next daily puzzle arrives at local midnight.'}else endPanel.classList.add('hidden');
 $('#legendText').innerHTML=isHard()?'<span class="swatch green"></span> Exact match. <span class="swatch yellow"></span> Season is one <em>event</em> away chronologically or placement is within 2. Gray means farther/different. Format is Full or Mini.':'<span class="swatch green"></span> Exact match. <span class="swatch yellow"></span> Season is 1 away or placement is within 2. Gray means farther/different.';
}
...
function statsKey(){return isHard()?'rydle_hard_stats_v1':'stcdle_stats_v1'}
...
function updateStats(won){if(isPractice())return;const today=dateKey(),s=getStats();if(s.lastCompleted===today)return;const yesterday=dateKey(new Date(Date.now()-DAY));s.played++;if(won){s.wins++;s.streak=s.lastCompleted===yesterday?s.streak+1:1;s.maxStreak=Math.max(s.maxStreak,s.streak);s.distribution[guesses.length-1]++}else s.streak=0;s.lastCompleted=today;localStorage.setItem(statsKey(),JSON.stringify(s))}
function renderStats(){const oldMode=mode==='practice'?'daily':mode==='hardPractice'?'hard':mode,s=JSON.parse(localStorage.getItem(oldMode==='hard'?'rydle_hard_stats_v1':'stcdle_stats_v1')||'{"played":0,"wins":0,"streak":0,"maxStreak":0,"lastCompleted":null,"distribution":[0,0,0,0,0,0,0,0]}');$('#statsTitle').textContent=oldMode==='hard'?'Hard statistics':'Daily statistics';$('#statPlayed').textContent=s.played;$('#statWin').textContent=s.played?Math.round(s.wins/s.played*100)+'%':'0%';$('#statStreak').textContent=s.streak;$('#statMax').textContent=s.maxStreak;const box=$('#distribution');box.innerHTML='';const max=Math.max(1,...s.distribution);s.distribution.forEach((n,i)=>{const r=document.createElement('div');r.className='dist-row';r.innerHTML=`<span>${i+1}</span><div class="dist-bar" style="width:${Math.max(8,n/max*100)}%">${n}</div>`;box.appendChild(r)})}
function emojiResult(){const won=guesses.at(-1)?.id===answer.id;const title=mode==='hard'?`Rydle Hard #${Math.max(1,puzzleNumber())} ${won?guesses.length:'X'}/8`:mode==='daily'?`Rydle #${Math.max(1,puzzleNumber())} ${won?guesses.length:'X'}/8`:mode==='hardPractice'?`Rydle Hard Practice ${won?guesses.length:'X'}/8`:`Rydle Practice ${won?guesses.length:'X'}/8`;const lines=guesses.map(g=>compare(g).map(([,c])=>c==='green'?'🟩':c==='yellow'?'🟨':'⬛').join(''));return `${title}\n\n${lines.join('\n')}`}
...
document.querySelectorAll('.tab').forEach(b=>b.addEventListener('click',()=>{mode=b.dataset.mode;message.textContent='';input.value='';selected=null;if(isPractice())randomPractice();else loadDaily();render()}));
